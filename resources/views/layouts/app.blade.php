...
<body class="font-sans antialiased bg-gray-100"
    ...
    <div class="flex h-screen bg-gray-100 overflow-hidden">
        ...
        {{-- ════════════════════════════════════
             SIDEBAR — Desktop colapsa, Mobile drawer
        ════════════════════════════════════ --}}
        <aside x-cloak
            ...
            <div class="flex items-center justify-between h-16 border-b border-gray-100 px-4 shrink-0">
                ...
                <button @click="sidebarOpen = !sidebarOpen"
                        class="hidden md:flex text-gray-400 hover:text-blue-600 w-7 h-7 items-center justify-center rounded-lg hover:bg-blue-50 transition-all">
            ...
            <div class="flex-1 flex flex-col overflow-y-auto sidebar-scroll py-3 space-y-0.5 overflow-x-hidden">
                ...
                <a href="{{ route('dashboard') }}"
                   ...
                   :class="'{{ request()->routeIs('dashboard') }}'
                    ...
                    <div class="w-8 h-8 flex items-center justify-center rounded-lg shrink-0"
                         :class="'{{ request()->routeIs('dashboard') }}' ? 'bg-blue-500' : 'group-hover:bg-white'">
                ...
                <div x-data="{ open: {{ request()->routeIs('sales.*') ? 'true' : 'false' }} }" class="mx-2">
                    <button @click="open = !open; if(!sidebarOpen && !isMobile) sidebarOpen = true"
                        ...
                        <div class="flex items-center">
                            <div class="w-8 h-8 flex items-center justify-center rounded-lg bg-blue-50 group-hover:bg-blue-100 shrink-0">
                    ...
                    <div x-show="open && (sidebarOpen || isMobile)" x-collapse class="mt-0.5 space-y-0.5">
                        <a href="{{ route('sales.create') }}"
                           ...
                           class="flex items-center pl-11 pr-3 py-2 text-sm rounded-xl transition-all
                                  {{ request()->routeIs('sales.create') ? 'text-blue-600 bg-blue-50 font-semibold' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800' }}">
                            <i class="fas fa-plus-circle w-4 mr-2 text-xs"></i> Nueva Venta
                        </a>
                        <a href="{{ route('sales.index') }}"
                           @click="if(isMobile) mobileOpen = false"
                           class="flex items-center pl-11 pr-3 py-2 text-sm rounded-xl transition-all
                                  {{ request()->routeIs('sales.index') ? 'text-blue-600 bg-blue-50 font-semibold' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800' }}">
                ...
                <a href="{{ route('reports.index') }}"
                   ...
                   :class="'{{ request()->routeIs('reports.*') }}'
                       ...
                       : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'">
                    <div class="w-8 h-8 flex items-center justify-center rounded-lg shrink-0"
                         :class="'{{ request()->routeIs('reports.*') }}' ? 'bg-purple-500' : 'bg-purple-50 group-hover:bg-purple-100'">
                        <i class="fas fa-chart-pie text-sm" :class="'{{ request()->routeIs('reports.*') }}' ? 'text-white' : 'text-purple-500'"></i>
                    </div>
                    <span class="ml-3 font-semibold text-sm whitespace-nowrap"
            ...
            <div class="shrink-0 p-3 border-t border-gray-100 bg-gray-50">
                <div class="flex items-center gap-3 px-2 py-2 mb-2" x-show="sidebarOpen || isMobile">
                    <div class="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-black shrink-0">
        ...
        <div class="flex-1 flex flex-col overflow-hidden min-w-0">
            ...
            <header class="md:hidden bg-white border-b border-gray-200 shadow-sm shrink-0 z-30">
                <div class="flex items-center justify-between px-4 h-14">
                    <button @click="mobileOpen = true"
                            class="w-9 h-9 flex items-center justify-center rounded-xl bg-gray-100 hover:bg-blue-50 hover:text-blue-600 text-gray-500 transition-all active:scale-95">
                    ...
                    <img src="{{ asset('img/logo.png') }}"
                         ...
                         onerror="this.style.display='none'">
                    <div class="w-9 h-9 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-black">
                        {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                    </div>
                </div>
            </header>
            ...
            ════════════════════════════════════ --}}
            <nav class="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-gray-200 shadow-lg safe-area-inset-bottom">
                <div class="flex items-center justify-around px-2 py-1.5">
                    ...
                    <a href="{{ route('sales.create') }}"
                       class="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all
                              {{ request()->routeIs('sales.create') ? 'text-blue-600' : 'text-gray-400' }}">
                        <div class="w-8 h-8 flex items-center justify-center rounded-xl transition-all
                                    {{ request()->routeIs('sales.create') ? 'bg-blue-100' : '' }}">
                            <i class="fas fa-plus-circle text-base"></i>
                        </div>
                        <span class="text-[10px] font-bold">Nueva Venta</span>
                    </a>

                    {{-- Botón central — menú completo --}}
                    <button @click="mobileOpen = true"
                            class="flex flex-col items-center gap-0.5 -mt-5">
                        <div class="w-12 h-12 rounded-2xl bg-blue-600 shadow-lg shadow-blue-200 flex items-center justify-center active:scale-95 transition-transform">
                    ...
                    <a href="{{ route('sales.index') }}"
                       class="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all
                              {{ request()->routeIs('sales.index') ? 'text-blue-600' : 'text-gray-400' }}">
                        <div class="w-8 h-8 flex items-center justify-center rounded-xl transition-all
                                    {{ request()->routeIs('sales.index') ? 'bg-blue-100' : '' }}">
    ...
    <script>
        ...
        @if ($errors->any())
            ...
            Swal.fire({
                ...
                confirmButtonText: 'Entendido', confirmButtonColor: '#3B82F6',
            });
        @endif
