        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    ...
                    <h1 class="text-2xl font-black text-gray-900 tracking-tight">Nueva Venta de Vales</h1>
                </div>
                <p class="text-gray-400 text-sm pl-4">Logística automática basada en unidades del cliente</p>
            </div>
            <div class="flex items-center gap-3">
                {{-- Indicadores de paso --}}
                <div class="flex items-center gap-1.5 bg-white border border-gray-100 rounded-2xl px-4 py-2.5 shadow-sm">
                    <div class="flex items-center gap-1.5">
                        <div class="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center text-[10px] font-black text-white">1</div>
                        <span class="text-[11px] font-bold text-blue-600 hidden sm:block">Cliente</span>
                    </div>
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <span class="text-[11px] font-bold hidden sm:block transition-colors"
                ...
                <div class="bg-white border border-gray-100 rounded-2xl px-4 py-2.5 shadow-sm text-right">
                    <span class="block text-[9px] font-black text-gray-300 uppercase tracking-widest">Folio</span>
                    <span class="text-base font-black font-mono text-blue-600">#VTA-NUEVA</span>
                </div>
            </div>
        </div>
                ...
                <form id="saleForm" action="{{ route('sales.store') }}" method="POST" @submit.prevent="submitForm">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-50 flex items-center gap-3">
                            <div class="w-7 h-7 rounded-xl bg-blue-600 flex items-center justify-center text-xs font-black text-white shrink-0">1</div>
                            <h3 class="text-sm font-black text-gray-800 uppercase tracking-wide">Datos del Cliente</h3>
                        </div>
                        <div class="p-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div class="space-y-1.5">
                                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest">Cliente</label>
                                    <select name="client_id" x-model="clientId" @change="onClientChange()" required
                                            class="w-full border-gray-200 focus:border-blue-500 focus:ring-blue-500 rounded-xl text-sm font-medium transition-colors bg-gray-50 focus:bg-white">
                                ...
                                <div class="space-y-1.5">
                                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest">RFC</label>
                                    <div class="relative">
                                        <input type="text" x-model="clientRfc" placeholder="—" readonly
                                               class="w-full bg-gray-50 border-gray-100 rounded-xl text-sm text-gray-500 cursor-not-allowed font-mono">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between gap-3">
                            <div class="flex items-center gap-3">
                                <div class="w-7 h-7 rounded-xl flex items-center justify-center text-xs font-black shrink-0 transition-colors"
                                     :class="confirmedLines.length > 0 ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'">2</div>
                                <h3 class="text-sm font-black text-gray-800 uppercase tracking-wide">Materiales a Facturar</h3>
                            </div>
                            <div x-show="confirmedLines.length > 0" x-transition
                                 class="flex items-center gap-1.5 bg-blue-50 text-blue-600 text-[10px] font-black px-3 py-1.5 rounded-full">
                        ...
                        <div class="p-6 space-y-5">
                            ...
                            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-5">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                                    <div class="md:col-span-4 space-y-1.5">
                                        <label class="block text-[10px] font-black text-gray-500 uppercase tracking-wide">Material</label>
                                        <select id="activeMaterialSelect" x-model="draft.material_id" @change="onDraftMatChange()"
                                                class="w-full border-blue-200 bg-white focus:border-blue-500 focus:ring-blue-500 rounded-xl text-sm font-medium transition-colors">
                                    ...
                                    <div class="md:col-span-2 space-y-1.5">
                                        <label class="block text-[10px] font-black text-gray-500 uppercase tracking-wide">Cantidad</label>
                                        <div class="relative">
                                            <input type="number" step="0.01" x-model.number="draft.cantidad" placeholder="0.00"
                                                   class="w-full border-blue-200 bg-white focus:border-blue-500 focus:ring-blue-500 rounded-xl text-sm font-black text-center pr-7 transition-colors">
                                            <span class="absolute right-2.5 top-1/2 -translate-y-1/2 text-[9px] font-black text-blue-300 pointer-events-none" x-text="draft.unitLabel"></span>
                                        </div>
                                    </div>
                                    <div class="md:col-span-2 space-y-1.5">
                                        <label class="block text-[10px] font-black text-gray-500 uppercase tracking-wide">Precio</label>
                                        <div class="relative">
                                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-black text-gray-300">$</span>
                                            <input type="number" step="0.01" x-model="draft.precio" readonly
                                                   class="w-full bg-gray-100 border-gray-100 rounded-xl text-sm text-center text-gray-400 cursor-not-allowed pl-6">
                                        </div>
                                    </div>
                                    <div class="md:col-span-2 space-y-1.5">
                                        <label class="block text-[10px] font-black text-gray-500 uppercase tracking-wide">Desc. %</label>
                                        <div class="relative">
                                            <input type="number" x-model="draft.descuentoPorcentaje" placeholder="0"
                                                   class="w-full border-blue-200 bg-white focus:border-blue-500 focus:ring-blue-500 rounded-xl text-sm text-center transition-colors pr-6">
                                            <span class="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-gray-300">%</span>
                                        </div>
                                    </div>
                                    <div class="md:col-span-2">
                                        <button type="button" @click="confirmDraft()"
                                                ...
                                                class="w-full flex items-center justify-center gap-2 font-black text-xs py-2.5 px-4 rounded-xl transition-all duration-150 shadow-sm">
                                ...
                                <div class="mt-3 flex justify-end" x-show="draft.material_id && parseFloat(draft.cantidad) > 0" x-transition>
                                    <span class="text-[11px] text-blue-500 font-black bg-white border border-blue-100 px-3 py-1 rounded-full shadow-sm">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                                    <template x-for="(line, idx) in confirmedLines" :key="idx">
                                        <button type="button" @click="openModal(idx)"
                                                class="group text-left bg-gray-50 hover:bg-white border border-gray-100 hover:border-blue-200 hover:shadow-md rounded-xl p-3.5 transition-all duration-200 hover:-translate-y-0.5 relative overflow-hidden">
                                            <div class="absolute left-0 top-0 bottom-0 w-0.5 bg-blue-500 rounded-l-xl"></div>
                                            <div class="pl-3 pr-7">
                                                <div class="flex items-center justify-between gap-2 mb-1.5">
                                                    <p class="text-xs font-black text-gray-700 group-hover:text-blue-700 truncate leading-tight" x-text="getMaterialName(line.material_id)"></p>
                                                    <span class="shrink-0 text-[9px] font-black bg-gray-100 group-hover:bg-blue-100 text-gray-400 group-hover:text-blue-500 px-1.5 py-0.5 rounded-md transition-colors" x-text="line.unitLabel"></span>
                                                </div>
                                                <div class="flex items-center justify-between">
                                                    <span class="text-[11px] text-gray-400 font-medium">
                                                        ...
                                                        <template x-if="parseFloat(line.descuentoPorcentaje) > 0">
                                                            <span class="ml-1 text-red-400 font-black">−<span x-text="line.descuentoPorcentaje"></span>%</span>
                                                        </template>
                                                    </span>
                                                    <span class="text-xs font-black text-gray-800">$<span x-text="lineSubtotal(line).toFixed(2)"></span></span>
                                                </div>
                                            </div>
                                            <div class="absolute right-2.5 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <div x-show="confirmedLines.length === 0" class="flex flex-col items-center justify-center py-8 text-center border-2 border-dashed border-gray-100 rounded-2xl bg-gray-50/50">
                                ...
                                <p class="text-xs font-black text-gray-300 uppercase tracking-wide">Sin materiales</p>
                                <p class="text-[11px] text-gray-300 mt-1">Selecciona y presiona Agregar</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between gap-3 flex-wrap">
                            <div class="flex items-center gap-3">
                                <div class="w-7 h-7 rounded-xl flex items-center justify-center text-xs font-black shrink-0 transition-colors"
                                     :class="trips.length > 0 && remaining <= 0 ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400'">3</div>
                                <h3 class="text-sm font-black text-gray-800 uppercase tracking-wide">Logística de Entregas</h3>
                            </div>
                            <div class="flex items-center gap-2 flex-wrap">
                                <div class="flex items-center bg-gray-100 p-0.5 rounded-xl">
                                    <button type="button" @click="setMode('auto')"
                                            ...
                                            class="px-3.5 py-1.5 text-[11px] font-black rounded-[10px] transition-all">⚡ Auto</button>
                                    <button type="button" @click="setMode('manual')"
                                            :class="mode === 'manual' ? 'bg-white shadow text-blue-600' : 'text-gray-400 hover:text-gray-600'"
                                            class="px-3.5 py-1.5 text-[11px] font-black rounded-[10px] transition-all">✋ Manual</button>
                                </div>
                                <select name="tipo_venta" class="border-gray-200 rounded-xl text-xs font-bold focus:border-blue-500 focus:ring-blue-500 bg-gray-50 py-2">
                            ...
                            <div class="bg-gray-50 border border-gray-100 rounded-xl p-4">
                                <div class="flex items-center justify-between mb-2.5">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-[11px] font-black text-red-500 uppercase tracking-wide" x-show="remaining > 0">
                                            Falta: <span x-text="remaining.toFixed(2)"></span> <span x-text="primaryUnitLabel"></span>
                                        </span>
                                        <span class="text-[11px] font-black text-green-600 uppercase tracking-wide" x-show="remaining <= 0 && trips.length > 0">✓ Logística completa</span>
                                        <span class="text-[11px] font-black text-gray-300 uppercase tracking-wide" x-show="trips.length === 0">Esperando materiales...</span>
                                    </div>
                                    <span class="text-[11px] font-black text-gray-400">
                                        <span class="text-blue-600" x-text="totalDistributed.toFixed(2)"></span> / <span x-text="totalCantidad.toFixed(2)"></span>
                                    </span>
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div x-show="mode === 'manual'" class="border border-gray-100 rounded-xl overflow-hidden bg-white flex flex-col">
                                    <div class="px-4 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between shrink-0">
                                        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Flotilla</span>
                                        <span class="text-[10px] font-black bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full" x-text="clientUnits.length + ' unidades'"></span>
                                    </div>
                                    ...
                                    <div class="px-3 pt-3 shrink-0">
                                        <button type="button" @click="addTrip(null, 'Externo / Flete', 0)"
                                            ...
                                            <span class="text-[11px] font-black text-gray-400 group-hover:text-blue-600 transition-colors">Unidad Externa / Flete</span>
                                        </button>
                                    </div>

                                    {{-- Grid chips 2 columnas --}}
                                    <div class="p-3 overflow-y-auto custom-scrollbar" style="max-height: 280px;">
                                        <div x-show="clientUnits.length === 0" class="flex flex-col items-center py-8 text-center">
                                            ...
                                            <p class="text-[11px] font-black text-gray-300">Sin unidades</p>
                                            <p class="text-[10px] text-gray-200 mt-0.5">Selecciona un cliente primero</p>
                                        </div>
                                            ...
                                            <template x-for="unit in clientUnits" :key="unit.id">
                                                <button type="button"
                                                    ...
                                                    <div class="flex items-center justify-between w-full mb-1">
                                                        <span class="text-[11px] font-black text-gray-700 group-hover:text-blue-700 transition-colors truncate pr-1" x-text="unit.placa"></span>
                                                    ...
                                                    <span class="text-[10px] text-gray-400 font-medium leading-tight truncate w-full" x-text="unit.tipo_vehiculo"></span>
                                                    <span class="text-[10px] font-black text-blue-400 mt-0.5">Cap: <span x-text="unit.capacidad_maxima"></span></span>
                                                </button>
                                            </template>
                                        </div>
                                    </div>
                                ...
                                <div class="border border-gray-100 rounded-xl overflow-hidden flex flex-col bg-gray-50"
                                    ...
                                    <div class="px-4 py-3 bg-white border-b border-gray-100 flex items-center justify-between shrink-0">
                                        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Viajes Programados</span>
                                        <span class="text-[10px] font-black px-2 py-0.5 rounded-full transition-colors"
                                    ...
                                    <div class="p-3 overflow-y-auto custom-scrollbar space-y-2" style="max-height: 320px;">
                                        <template x-for="(trip, index) in trips" :key="index">
                                            <div class="flex items-center gap-3 bg-white border border-gray-100 rounded-xl p-3 group hover:border-blue-100 hover:shadow-sm transition-all relative overflow-hidden">
                                                ...
                                                <div class="pl-2 flex-1 min-w-0">
                                                    <p class="text-[11px] font-black text-gray-700 truncate mb-1.5" x-text="trip.name"></p>
                                                    <div class="flex items-center gap-2">
                                                        <span class="text-[9px] font-black text-gray-400 uppercase tracking-wide shrink-0">Carga:</span>
                                                        <input type="number" step="0.01" x-model="trip.amount"
                                                               ...
                                                               class="w-20 p-1 text-xs rounded-lg text-center font-black border transition-colors">
                                                        <span class="text-[10px] font-black text-gray-300" x-text="primaryUnitLabel"></span>
                                                    </div>
                                                </div>
                                        ...
                                        <div x-show="trips.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
                                            ...
                                            <p class="text-xs font-black text-gray-300" x-show="totalCantidad <= 0">Agrega materiales primero</p>
                                            <p class="text-xs font-black text-gray-300" x-show="totalCantidad > 0 && clientUnits.length === 0 && mode === 'auto'">Sin unidades — usa modo Manual</p>
                                            <p class="text-xs font-black text-gray-300" x-show="totalCantidad > 0 && (clientUnits.length > 0 || mode === 'manual')">Selecciona unidades de la flotilla</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 sticky top-6 overflow-hidden">
                    <div class="px-5 py-4 border-b border-gray-50 flex items-center gap-3">
                        ...
                        <h4 class="text-sm font-black text-gray-800 uppercase tracking-wide">Resumen</h4>
                    </div>
                    ...
                    <div class="p-5">
                            ...
                            <template x-for="(line, idx) in confirmedLines" :key="idx">
                                <div class="flex justify-between items-start gap-2 text-xs" x-show="line.material_id">
                                    <div class="flex items-start gap-2 min-w-0">
                                        ...
                                        <span class="text-gray-500 font-medium truncate" x-text="getMaterialName(line.material_id)"></span>
                                    </div>
                                    <span class="font-black text-gray-800 shrink-0">$<span x-text="lineSubtotal(line).toFixed(2)"></span></span>
                                </div>
                            </template>
                            <div x-show="confirmedLines.length === 0" class="text-[11px] text-gray-300 font-black text-center py-3 uppercase tracking-wide">
                                Sin materiales
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 text-xs border-t border-gray-50 pt-4">
                            <div class="flex justify-between text-gray-400">
                                ...
                                <span class="font-bold">$<span x-text="importeBruto.toFixed(2)"></span></span>
                            </div>
                            <div class="flex justify-between text-red-400 font-black" x-show="montoDescuento > 0" x-transition>
                            ...
                            <div class="flex justify-between text-gray-600">
                                ...
                                <span class="font-black">$<span x-text="subtotal.toFixed(2)"></span></span>
                            </div>
                            <div class="flex justify-between text-gray-400">
                                <span class="font-medium">IVA 16%</span>
                                <span class="font-bold">$<span x-text="iva.toFixed(2)"></span></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-blue-600 px-5 py-4 flex justify-between items-center">
                        ...
                        <span class="text-2xl font-black text-white">$<span x-text="total.toFixed(2)"></span></span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4 flex-wrap">
                ...
                <div class="flex items-center gap-3 flex-wrap flex-1">
                    <div class="flex items-center gap-1.5">
                        ...
                        <span class="text-[11px] font-black text-gray-400">
                            <span class="text-gray-700" x-text="confirmedLines.length"></span> material(es)
                        </span>
                    </div>
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <span class="text-[11px] font-black text-gray-400">
                            <span class="text-gray-700" x-text="trips.length"></span> viaje(s)
                        </span>
                    </div>
                    ...
                    <div class="flex items-center gap-1.5" x-show="totalCantidad > 0">
                        ...
                        <span class="text-[11px] font-black transition-colors" :class="remaining <= 0 ? 'text-green-600' : 'text-red-400'">
                ...
                <div class="flex items-center gap-4 ml-auto">
                    <div class="text-right hidden sm:block">
                        <p class="text-[9px] font-black text-gray-300 uppercase tracking-widest">Total</p>
                        <p class="text-xl font-black text-blue-600">$<span x-text="total.toFixed(2)"></span></p>
                    </div>
                    <button type="button" @click="submitForm()"
                            ...
                            class="flex items-center gap-2 font-black py-2.5 px-5 rounded-xl shadow transition-all duration-200 text-sm whitespace-nowrap">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-show="modalOpen"
        ...
        <div x-show="modalOpen"
            ...
            <div class="bg-blue-600 px-5 py-4 flex items-start justify-between gap-4">
                <div>
                    <p class="text-[9px] font-black text-blue-200 uppercase tracking-widest mb-0.5">Detalle del material</p>
                    <h3 class="text-base font-black text-white leading-tight" x-text="modalLine ? getMaterialName(modalLine.material_id) : ''"></h3>
                </div>
            ...
            <div class="p-5 space-y-3" x-show="modalLine">
                <div class="grid grid-cols-2 gap-3">
                    <div class="bg-gray-50 rounded-xl p-3.5 text-center border border-gray-100">
                        <p class="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Cantidad</p>
                        <p class="text-xl font-black text-gray-800" x-text="modalLine ? modalLine.cantidad : ''"></p>
                        <p class="text-[10px] font-black text-gray-300 mt-0.5" x-text="modalLine ? modalLine.unitLabel : ''"></p>
                    </div>
                    <div class="bg-gray-50 rounded-xl p-3.5 text-center border border-gray-100">
                        <p class="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Precio Unit.</p>
                        <p class="text-xl font-black text-gray-800">$<span x-text="modalLine ? parseFloat(modalLine.precio).toFixed(2) : ''"></span></p>
                        <p class="text-[10px] font-black text-gray-300 mt-0.5">por <span x-text="modalLine ? modalLine.unitLabel : ''"></span></p>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 rounded-xl p-3 border border-gray-100 flex items-center justify-between"
                    ...
                    <span class="text-xs font-black text-gray-400">Descuento</span>
                    <div class="text-right">
                        <span class="text-sm font-black text-red-500"><span x-text="modalLine ? modalLine.descuentoPorcentaje : ''"></span>%</span>
                        <p class="text-[10px] text-gray-400">−$<span x-text="modalLine ? (parseFloat(modalLine.cantidad)*parseFloat(modalLine.precio)*(parseFloat(modalLine.descuentoPorcentaje)/100)).toFixed(2) : ''"></span></p>
                    </div>
                </div>

                <div class="bg-blue-600 rounded-xl p-3.5 flex justify-between items-center">
                    <p class="text-xs font-black text-blue-200">Subtotal línea</p>
                    <p class="text-xl font-black text-white">$<span x-text="modalLine ? lineSubtotal(modalLine).toFixed(2) : ''"></span></p>
                </div>

                <div class="flex gap-2.5 pt-1">
                    <button type="button" @click="removeConfirmedLine(modalLineIdx); closeModal()"
                            class="flex-1 flex items-center justify-center gap-2 bg-red-50 hover:bg-red-100 text-red-500 font-black text-xs py-2.5 rounded-xl transition-colors">
                    ...
                    <button type="button" @click="closeModal()"
                            class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs py-2.5 rounded-xl transition-colors">
                        Cerrar
                    </button>
                </div>
            </div>
